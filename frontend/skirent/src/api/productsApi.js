...
// ✅ חשוב: הוספתי "/" בסוף כדי למנוע redirect+CORS

export async function takeProduct(productId, qty = 1) {
  const res = await api.post(`/products/${productId}/take/`, { qty: Number(qty) });
  return res.data;
}

export async function returnTakenProduct(productId, qty = 1) {
  const res = await api.post(`/products/${productId}/return-taken/`, { qty: Number(qty) });
  return res.data;
}
...
export async function returnRentedProduct(productId, qty = 1) {
  const res = await api.post(`/products/${productId}/return-rented/`, { qty: Number(qty) });
  return res.data;
}
